...
def paginate(request, totalQuery):
    ...
    totalCount = 300
    pages = int(math.ceil(totalCount / pagesize))
...
def lncrnaStructure(request, lncrnaid):
    lncrnaRecord = lncrna_info.objects.using('human').get(pk=lncrnaid)
    editingSites = edit_site_info.objects.using('human').filter(lncrna_info_id__exact=lncrnaid)
...
def functionGainLoss(request, function, editingid, mirnaid):
    ...
    editingLoss = None
    try:
        editingGain = function_gains.objects.using('human').filter(edit_site_info_id=editingid).get(mirna_info_id__exact=mirnaid)
    except:
        pass
    try:
        editingLoss = function_losses.objects.using('human').filter(edit_site_info_id=editingid).get(mirna_info_id__exact=mirnaid)
    except:
        pass
    ...
    if function == "gain":
        return render(request, 'human/function_gain.html', context)
    if function == "loss":
        return render(request, 'human/function_loss.html', context)

def rnafold(request, lncrnaid, editingid):
    title = "Human RNA fold"
    lncrnafold = lncrna_info.objects.using('human').get(pk=lncrnaid)
    editingfold = edit_site_info.objects.using('human').get(pk=editingid)

    context = {"title": title, "lncrnaid": lncrnaid, "editingid":editingid, "lncrnafold":lncrnafold, "editingfold":editingfold}

    return render(request, 'human/rnafold.html', context)
...
def quickSearch(request):
    ...
    if positionPattern.search(term):
        ...
        total = mirna_info.objects.using('human')
    ...
    return render(request, 'human/search.html', context)
...
def tInteraction(request):
    ...
    if not wild:
        return redirect('http://bioinfo.life.hust.edu.cn/LNCediting/')
    position = int(request.GET.get('position').strip())
    edit = wild[:position - 1] + "G" + wild[position:]
    uniq_code = uuid.uuid4().hex.upper()
    command = "perl %s/templates/tools/tool_list/interaction/online_analysis.pl %s %s %s" %(functionPath, wild, edit, uniq_code)
    os.system(command)

    #Wild file
    file_wild = functionPath + "/templates/tools/tool_result/interaction_result/seq1_target." + uniq_code
    copyfile(file_wild, functionPath + '/static/human/download/wild_download.txt')
    copyfile(file_wild, settings.BASE_DIR + '/static/human/download/wild_download.txt')
    file_wild_list = list()
    ...
    #Edit file
    file_edit = functionPath + "/templates/tools/tool_result/interaction_result/seq2_target." + uniq_code
    copyfile(file_edit, functionPath + '/static/human/download/edit_download.txt')
    copyfile(file_edit, settings.BASE_DIR + '/static/human/download/edit_download.txt')
    file_edit_list = list()
    ...
    #Loss file
    file_loss = functionPath + "/templates/tools/tool_result/interaction_result/seq_target_loss." + uniq_code
    copyfile(file_loss, functionPath + '/static/human/download/loss_download.txt')
    copyfile(file_loss, settings.BASE_DIR + '/static/human/download/loss_download.txt')
    file_loss_list = list()
    ...
    #Gain file
    file_gain = functionPath + "/templates/tools/tool_result/interaction_result/seq_target_gain." + uniq_code
    copyfile(file_gain, functionPath + '/static/human/download/gain_download.txt')
    copyfile(file_gain, settings.BASE_DIR + '/static/human/download/gain_download.txt')
    file_gain_list = list()
    ...
    return render(request, 'tools/tool_result/miRNA_lncRNA_result.html', context)
...
def tStructure(request):
    ...
    position = int(request.GET.get('position').strip())
    edit = wild[:position-1] + "G" + wild[position:]
    command = "perl %s/templates/tools/tool_list/structure/online_analysis.pl %s %s" %(functionPath, wild, edit)
    os.system(command)

    #copy to human/static/human
    copyfile(functionPath + "/templates/tools/tool_list/structure/SEQ1.png", functionPath + "/static/human/tool_structure/SEQ1.png")
    copyfile(functionPath + "/templates/tools/tool_list/structure/SEQ2.png", functionPath + "/static/human/tool_structure/SEQ2.png")
    #copy to static/human in root path

    copyfile(functionPath + "/templates/tools/tool_list/structure/SEQ1.png", settings.BASE_DIR + "/static/human/tool_structure/SEQ1.png")
    copyfile(functionPath + "/templates/tools/tool_list/structure/SEQ2.png", settings.BASE_DIR + "/static/human/tool_structure/SEQ2.png")

    file_wild = "%s/templates/tools/tool_list/structure/seq1_RNAfold_re" % functionPath
    ...
    file_editing = "%s/templates/tools/tool_list/structure/seq2_RNAfold_re" % functionPath
    ...
    context = {"titile": title, "wild":wild, "position":position, "edit":edit, "fe_wild_sequence":fe_wild_sequence, "fe_wild_notation":fe_wild_notation, "fe_wild_energy": fe_wild_energy, "fe_editing_sequence":fe_editing_sequence, "fe_editing_notation":fe_editing_notation,"fe_editing_energy":fe_editing_energy}

    return render(request,'tools/tool_result/editing_lncRNA_result.html',context)
